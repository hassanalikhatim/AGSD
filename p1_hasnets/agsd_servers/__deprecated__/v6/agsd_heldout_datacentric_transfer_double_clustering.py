...
class Server_HaSNet_from_HeldOut(Server):
    ...
    def rescaled_noisy_aggregate(self, clients_states: list[dict], scalers: np.ndarray=None, strength=1e-5):
        ...
        for key in self.model.model.state_dict().keys():
            ...
            delta_state_t[key] = torch.median(torch.stack([(client_state[key]-self.model.model.state_dict()[key]).clone()*scalers[i] for i, client_state in enumerate(clients_states)], dim=0), dim=0)[0]
    ...
    def get_std_and_confidence_torch(self, all_models: list[dict], **kwargs):
        ...
        for model_ in all_models:
            ...
            loss_.append(diff_norm(act_y, adv_y))
    ...
    def aggregate_defended_torch(self, clients_state_dict: list[dict], **kwargs):
        ...
        # processing nan values in the trust index
        if torch.sum(torch.isnan(self_gamma)) > 0: self_gamma = torch.zeros_like(self_gamma)
        ...
        # self_cs_values +=  non_weight_of_this_value(self_cs_values) * torch.matmul(self_gamma.view(-1, 1), torch.t(self_gamma.view(-1, 1)))
        ...
        self.kmeans.fit(self.gamma.reshape(-1, 1))
        self.labels_2 = self.kmeans.labels_
        sorted_labels_2 = np.sort(np.unique(self.labels_2))
        def get_best_label_2(values_: np.ndarray):
            _a_means = np.array([np.mean(values_[np.where(self.labels_2==label)]) for label in sorted_labels_2])
            return sorted_labels_2[np.argmax(_a_means).reshape(-1)[0]]
        best_label_2 = get_best_label_2(self.gamma)
        
        best_condition = (self.labels_==best_label) & (self.labels_2==best_label_2)
        worst_condition = (self.labels_!=best_label)# & (self.labels_2!=best_label_2)
        ...
        def get_clean_selected_ratio(arr_: np.ndarray):
            best_condition = self.labels_==get_best_label(arr_)
        ...
        values_dict = {
            ...
            # 'h': normalized_torch(gamma_0).detach().cpu().numpy(),
        }
        ...
        # self.a_msg_that_i_need_to_print += '|' + get_max_mean_min(normalized_torch(loss_).detach().cpu().numpy(), 'L')
