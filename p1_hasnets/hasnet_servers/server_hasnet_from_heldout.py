...
class Server_HaSNet_from_HeldOut(Server):
    ...
    def process_and_aggregate(self, clients_states: list[dict], strength=1e-5):
        ...
        norms = torch.norm(flattened_clients_states-initial_model_state, p=2, dim=1)
        ...
        for key in self.model.model.state_dict().keys():
            ...
            for i in range(len(clients_states)):
                if i==0:
                    delta_state_t[key] = (clients_states[i][key] - self.model.model.state_dict()[key]).clone() * scalers[i] / len(clients_states)
                else:
                    delta_state_t[key] += (clients_states[i][key] - self.model.model.state_dict()[key]).clone() * scalers[i] / len(clients_states)
            
            state_t[key] += delta_state_t[key]
    ...
    def rescale_and_flatten(self, clients_states: list[dict]):
        ...
        return flattened_clients_states * scalers
    ...
    def compute_gammas(self, clients_state_dict: list[dict]):
        ...
        perturbed_aggregated_state = self.process_and_aggregate(clients_state_dict)
    ...
    def fit_clusters(self):
        ...
        # self.processed_gamma = self.gamma
    ...
    def aggregate_defended(self, clients_state_dict: list[dict], **kwargs):
        ...
        if np.sum(self.hasnet_indicator==1) > 0:
            ...
            return self.process_and_aggregate(selected_client_states)
        else:
